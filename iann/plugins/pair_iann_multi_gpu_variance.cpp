...
Usage:
  ...
  pair_style iann/multi_gpu/variance painn model.pt 5.5
...
static int get_atomic_number_global(double mass) {
    ...
    throw std::runtime_error("[PAIR_IANN_MULTI_GPU_VARIANCE] Unknown atomic number for mass: " + std::to_string(mass));
}

/* ---------------------------------------------------------------------- */

PairIANNMultiGPUVariance::PairIANNMultiGPUVariance(LAMMPS *lmp) : Pair(lmp),
  ...
  local_gpus(0)
{
  ...
  if (comm->me == 0) {
    if (!cuda_path) {
      error->warning(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] CUDA_HOME environment variable not set");
    } else {
      // Check CUDA version
      ...
      if (version_file.is_open()) {
        ...
        error->message(FLERR, ("[PAIR_IANN_MULTI_GPU_VARIANCE] Found CUDA version: " + cuda_version).c_str());
      }
    }
    if (!ld_path) {
      error->warning(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] LD_LIBRARY_PATH environment variable not set");
    }
  }
  ...
  try {
    if (torch::cuda::is_available()) {
      ...
      std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Running on GPU " << assigned_gpu_id << " of Node " << node_id << ". Total number of GPUs is " << global_gpus << std::endl;
      ...
      try {
        ...
        if (test_positions[0][0].item<float>() == 1.0) {
          std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " successfully assigned to GPU " << assigned_gpu_id << " of Node " << node_id << std::endl;
        }
      } catch (const c10::Error& e) {
        std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " failed to assign to GPU " << assigned_gpu_id << " of Node " << node_id << " - test failed: " << e.what() << std::endl;
      }
      ...
      if (local_gpus > 0) {
        ...
        if (comm->me == 0) {
          if (use_multi_gpu) {
            error->message(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Multi-GPU mode enabled ");
          } else {
            error->message(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Single GPU mode enabled");
          }
        }
      } else {
        use_gpu = false;
        use_multi_gpu = false;
        if (comm->me == 0) {
          error->message(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] No GPUs available, using CPU mode");
        }
      }
    }
  } catch (const c10::Error& e) {
    ...
    this->local_gpus = 0;
    if (comm->me == 0) {
      std::string msg = "[PAIR_IANN_MULTI_GPU_VARIANCE] GPU test failed: ";
      msg += e.what();
  ...
  if (!use_gpu && comm->me == 0) {
    error->message(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Using CPU mode");
  }
}

/* ---------------------------------------------------------------------- */

PairIANNMultiGPUVariance::~PairIANNMultiGPUVariance() 
{
  if (allocated) {
...
void PairIANNMultiGPUVariance::compute(int eflag, int vflag) 
{
  ev_init(eflag, vflag);
  ...
  try {
      ...
      if (debug) {
        std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " Using multi-GPU inference with " << local_gpus << " GPUs per node" << std::endl;
      }
      ...
      if (debug && comm->me == 0) {
        ...
        if (printed) {
        std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Using single GPU inference" << std::endl;
        printed = false;
        }
      }
      ...
      if (forces_tensor.size(0) == nall) {
        ...
        msg << "[PAIR_IANN_MULTI_GPU_VARIANCE] Model returned forces of shape [" << forces_tensor.size(0)
      ...
      if (eflag_global) {
        if (debug && comm->me == 0) {
          std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Energy tensor dim: " << energy_tensor.dim() 
        ...
        if (has_atomic_energy) {
          ...
          if (debug && comm->me == 0) {
            std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " Atomic energy sum (nlocal): " << eng_vdwl << std::endl;
          }
        } else {
          // Model returned scalar energy - use directly
          eng_vdwl = energy_tensor.item<float>();
          if (debug && comm->me == 0) {
            std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Scalar energy: " << eng_vdwl << std::endl;
          }
        }
      }
      ...
      if (has_variances) {
        ...
        if (debug && comm->me == 0) {
          std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Energy variance: " << energy_variance << std::endl;
          std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Force variance: " << force_variance << std::endl;
          std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Max energy variance: " << max_energy_variance << std::endl;
          std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Max force variance: " << max_force_variance << std::endl;
        }
      }
    }
    
  } catch (const c10::Error& e) {
    error->all(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Error running ML model: " + std::string(e.what()));
  }
...
void PairIANNMultiGPUVariance::allocate() 
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairIANNMultiGPUVariance::settings(int narg, char **arg) 
{
  if (narg < 3) error->all(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Illegal pair_style command: need model type and model path");
  ...
  for (int i = iarg; i < narg; i++) {
    if (strcmp(arg[i], "debug") == 0) {
      debug = true;
      if (comm->me == 0)
        error->message(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Debug mode enabled");
    }
  }
  ...
  try {
    if (debug) {
      std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " Model loading check: use_gpu=" << use_gpu << ", local_gpus=" << local_gpus << std::endl;
    }
    ...
    if (use_gpu && local_gpus > 0) {
      ...
      if (debug) {
        std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " Model loaded on GPU " << assigned_gpu_id << " of Node " << node_id << std::endl;
      }
    } else {
      ...
      if (comm->me == 0) {
        error->message(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Model loaded on CPU");
      }
    }
  }
  catch (const c10::Error& e) {
    error->all(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Error loading ML model: " + std::string(e.what()));
  }
}

/* ---------------------------------------------------------------------- */

void PairIANNMultiGPUVariance::coeff(int narg, char **arg) 
{
  ...
  if (count == 0) error->all(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Incorrect args for pair coefficients");
}

/* ---------------------------------------------------------------------- */

void PairIANNMultiGPUVariance::init_style() 
{
...
double PairIANNMultiGPUVariance::init_one(int i, int j) 
{
  if (setflag[i][j] == 0) {
    error->all(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] All pair coeffs must be set for ML potentials");
  }
...
void PairIANNMultiGPUVariance::build_edges(int inum, int *ilist, int *numneigh, int **firstneigh)
{
  int nlocal = atom->nlocal;
  int nall = nlocal + atom->nghost;
...
void PairIANNMultiGPUVariance::run_inference_on_gpu(int assigned_gpu_id, int node_id, int atom_start, int edge_start, int edge_end)
{
  try {
    ...
    if (debug) {
      std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " processing " << nall << " atoms (nlocal=" << atom->nlocal 
    ...
    if (comm->me == 0) {
      std::string msg = "[PAIR_IANN_MULTI_GPU_VARIANCE] Error on rank " + std::to_string(comm->me) + " (GPU " + std::to_string(assigned_gpu_id) + "): ";
      msg += e.what();
      error->warning(FLERR, msg.c_str());
    }
...
void PairIANNMultiGPUVariance::collect_results_from_gpus()
{
  int nlocal = atom->nlocal;
  double **f = atom->f;
  ...
  try {
    ...
    if (debug) {
      std::cout << "[PAIR_IANN_MULTI_GPU_VARIANCE] Rank " << comm->me << " Results collection completed" 
                << " eng_vdwl: " << eng_vdwl 
    ...
    error->all(FLERR, "[PAIR_IANN_MULTI_GPU_VARIANCE] Error collecting results from GPUs: " + std::string(e.what()));
  }
}
